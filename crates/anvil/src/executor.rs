...
//          inline-vs-dequeue for search branches.
...
//              LengthMismatch` for run_partitioned slice disagreement. Job
//              panics abort via scope propagation (jobs are panic-free by
//              contract — clippy::panic = deny enforces this in our code).
...
// CEP:COST: region setup = N thread spawns + N deque allocations (CEP-1
//           init boundary); task dispatch = 1 deque pop or steal (CEP-0).
...
// CEP:TODO(main-agent): CEP-3: replace per-region scoped threads with a
//           persistent park-based pool to cut region setup cost.
...
///          FIFO from neighbors (oldest = biggest subtree).
/// CEP:STATUS: complete
...
pub fn run_fork_join<J: Job>(root: J, num_workers: usize) -> Result<(), ExecError> {
    ...
    std::thread::scope(|s| {
        for w in 0..num_workers {
            ...
            s.spawn(move || worker_loop(w, own, table, pending_ref));
        }
    });
...
/// CEP:COST: 1 pop or steal attempt per iteration.
